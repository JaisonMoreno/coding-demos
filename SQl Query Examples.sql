...
with cte as (
select date, paying_customer, sum(downloads) as nonpaying,
       lead(sum(downloads)) over(partition by date order by paying_customer) as paying
...
select date, nonpaying, paying
from cte
where nonpaying > paying and paying_customer = 'no'
...
with cte as (
select p.title, p.budget as budget, ceiling(sum((datediff(day, start_date, end_date)/365.0) * e.salary)) as tot_prorated_amt
from linkedin_projects p
left join linkedin_emp_projects e_p
    on p.id = e_p.project_id
left join linkedin_employees as e
    on e.id = e_p.emp_id
group by p.title, p.budget
)

select title, budget, tot_prorated_amt
from cte
where tot_prorated_amt > budget
...
left join linkedin_employees as e
    on e.id = e_p.emp_id
group by p.title, p.budget
having p.budget < ceiling(sum((datediff(day, start_date, end_date)/365.0) * e.salary))


/* 39. Find the email activity rank for each user. Email activity rank is defined by the total number of emails sent. The user with the highest number of emails sent will have a rank of 1, and so on. Output the user, total emails, and their activity rank.
